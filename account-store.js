(() => {
  ...
  async function authenticate(identity,password){const d=await API.login(identity,password);if(d.profile){profile=mapProfile(d.profile);transactions=[];orders=[];write();window.dispatchEvent(new CustomEvent('surae-account-ready',{detail:profile}));setTimeout(()=>load().catch(()=>{}),60);return profile}await load();return profile}
