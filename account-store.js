(() => {
  ...
  const CACHE_KEY='surae_account_cache_v3';
  ...
  async function register({name,email,mobile,password}){const d=await API.register({name,email,mobile,password});if(d.needsEmailConfirmation&&!d.session)return {needsEmailConfirmation:true,email};if(d.profile){profile=mapProfile(d.profile);transactions=[];orders=[];write();window.dispatchEvent(new CustomEvent('surae-account-ready',{detail:profile}));setTimeout(()=>load().catch(()=>{}),60);return profile}await load();return profile}
  async function authenticate(identity,password){const d=await API.login(identity,password);if(String(d.role||'').toLowerCase()==='admin'){API.logout();profile=null;transactions=[];orders=[];write();throw new Error('This is an administrator account. Please use Admin Login.')}if(d.profile){profile=mapProfile(d.profile);transactions=[];orders=[];write();window.dispatchEvent(new CustomEvent('surae-account-ready',{detail:profile}));setTimeout(()=>load().catch(()=>{}),60);return profile}await load();return profile}
